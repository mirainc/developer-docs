...
Before you can create an API key you must first create an API source for one of our [supported POS providers](/docs/on-brand-menus).

<Callout>Generating your own API keys is currently only available for [Toast](/docs/on-brand-menus/pos-integrations/toast). You will be given an API key by our team as part of the [Revel](/docs/on-brand-menus/pos-integrations/revel) setup process.</Callout>

## Create an API Key

API keys can be created and managed from the Developer Portal’s [API keys](https://developers.displai.ai/api-keys) section.

1. Click the “Create New…” button <DisplaiIcon name="add" />.
2. Select “API Key” <DisplaiIcon name="api-keys" />.
...
<Callout>**Known issue**: After creating an API key for a new Source, you may need to re-publish your menus before data is returned by the [groups](/docs/on-brand-menus/reference/get-groups) endpoint.</Callout>
